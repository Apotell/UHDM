...
def _get_declarations(name, type, vpi, card, real_type=''):
    ...
    var_name = config.make_var_name(name, card)
    func_name = config.make_func_name(name, card)

    if card == '1':
        if type == 'std::string':
            content.append(f'  std::string_view get{func_name}() const{final};')
            content.append(f'  bool set{func_name}(std::string_view data);')

        elif type in ['int16_t', 'uint16_t', 'int32_t', 'uint32_t', 'int64_t', 'uint64_t', 'bool']:
            content.append(f'  {type} get{func_name}() const{final} {{ return m_{var_name}; }}')
            content.append(f'  bool set{func_name}({type} data) {{\n    m_{var_name} = data;\n    return true;\n  }}')

        else:
            Type = config.make_class_name(type)
            content.append(f'  {Type}* get{func_name}(){final} {{ return m_{var_name}; }}')
            content.append(f'  const {Type}* get{func_name}() const{final} {{ return m_{var_name}; }}')
            content.append(f'  template <typename T> T* get{func_name}() {{ return any_cast<T>(m_{var_name}); }}')
            content.append(f'  template <typename T> const T* get{func_name}() const {{ return any_cast<T>(m_{var_name}); }}')
            content.append(f'  bool set{func_name}({Type}* data) {{\n    {check}m_{var_name} = data;\n    return true;\n  }}')

    elif card == 'any':
        Type = config.make_class_name(type)
        content.append(f'  {Type}Collection* get{func_name}() const {{ return m_{var_name}; }}')
        content.append(f'  template<typename T> {Type}Collection* get{func_name}(T) = delete;')
        content.append(f'  {Type}Collection* get{func_name}(bool createIfNull);')
        content.append(f'  bool set{func_name}({Type}Collection* data) {{\n    {check}if (m_{var_name} == nullptr) {{\n      m_{var_name} = data;\n      return true;\n    }}\n    return false;\n  }}')
...
def _get_implementations(classname, name, type, vpi, card):
    ...
    if card == '1':
        ...
        if vpi == 'vpiFullName':
            ...
            content.append( '    }')
            content.append( '  }')
            content.append(f'  return m_serializer->getSymbol(m_{varName});')
            content.append( '}')
        else:
            content.append(f'std::string_view {ClassName}::get{FuncName}() const {{')
...
def _get_deepClone_implementation(model, models):
    ...
    for key, value in model.allitems():
        if key in ['class', 'obj_ref', 'class_ref', 'group_ref']:
            ...
            # Unary relations
            if card == '1':
                if (ClassName in ['RefObj', 'RefVar']) and (varName == 'actualGroup'):
                ...
                elif (ClassName in ['RefTypespec']) and (varName == 'actualTypespec'):
                    includes.add('ElaboratorListener')
...
def _get_getByVpiName_implementation(model):
    ...
    for key, value in model.allitems():
        if key in ['class', 'obj_ref', 'class_ref', 'group_ref']:
            ...
            if card == '1':
                content.append(f'  if ((m_{varName} != nullptr) && (m_{varName}->getName().compare(name) == 0)) {{')
                content.append(f'    return m_{varName};')
                content.append( '  }')
            else:
                type = value.get('type')
                ...
                content.append(f'  if (m_{varName} != nullptr) {{')
                content.append(f'    for (const BaseClass *ref : *m_{varName}) {{')
                content.append(f'      if (ref->getName().compare(name) == 0) return ref;')
                content.append( '    }')
                content.append( '  }')
    ...
    for key, value in model.allitems():
        ...
        if card == '1':
            if type not in ['string', 'int16_t', 'uint16_t', 'int32_t', 'uint32_t', 'int64_t', 'uint64_t', 'bool']:
                ...
                content.append(f'  if (auto withT = with->Cast<{TypeName}>()) if (auto p = m_{varName}) {{')
                content.append(f'    if (p == what) m_{varName} = withT; else p->swap(what, with);')
                content.append( '  }')
        else:
            if type not in ['any', 'symbol']:
                includes.add(type)

            content.append(f'  if (auto c = m_{varName}) if (auto withT = with->Cast<{TypeName}>()) {{')
...
def _generate_one_class(model, models, templates):
    ...
    public_declarations.append('  get_by_vpi_type_return_t getByVpiType(int32_t type) const override;')
