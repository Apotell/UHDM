...
def _generate_copy_implementations(models):
  ...
  for model in models.values():
    ...
    if classname in ['bit_select']:
        implementations.append(f'  ExprEval eval(nullptr);')
        implementations.append( '  const Any* const parent = source->getParent();')
        implementations.append( '  Expr* val = nullptr;')
        implementations.append( '  if (eval.reduceExpr(source->getIndex(), parent, &val, true)) {')
        implementations.append(f'    std::string indexName = prettyPrint(val);')
        implementations.append( '    if (Any* indexVal = bindAny(indexName)) {')
        implementations.append(f'      if (eval.reduceExpr(any_cast<Expr>(indexVal), parent, &val, true)) {{')
        implementations.append( '        indexName = prettyPrint(val);')
