...
const Any* UhdmFinder::findInCollection(std::string_view name, RefType refType,
  ...
  for (const Any* c : *collection) {
    ...
    if (const EnumTypespec* const et = uhdm::getTypespec<EnumTypespec>(c)) {
      if (const Any* const actual = findInTypespec(name, refType, et)) {
        return actual;
      } else if (const Any* const actual = findInTypespec(shortName, refType, et)) {
        return actual;
      }
    }
  ...
  for (size_t i = 1, n = hp->getPathElems()->size(); i < n; ++i) {
    if (hp->getPathElems()->at(i) == object) {
      ...
      if (const RefObj* const ro1 = any_cast<RefObj>(previous)) {
        ...
        if (const Variable* const var = ro1->getActual<Variable>()) {
        ...
        } else if (const Net* const net = ro1->getActual<Net>()) {
          ...
          if (const LogicTypespec* ln = getTypespec<LogicTypespec>(net)) {
            ts = ln;
          } else if (const Module* mi = net->getParent<Module>()) {
